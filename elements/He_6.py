...
def he4nn_he6_g(T):
    ...
    base_rate = he_k * 4.04 * 10**(-11) * T9**(-2.) * math.exp(-9.585 / T9) * (
        + 1.0
        + 0.138 * T9 
        )
...
def he6g_he4nn(T):
    """Caughlan, 1988"""
    T9 = constants.to_norm_tempreture(T, units="T9")
    forw = he4nn_he6_g.__wrapped__(T) / constants.to_norm_time(1) / he_k
...
def He_6_equ(X, T):
    T9 = constants.to_norm_tempreture(T, units="T9")
    try:
        X_he6 = (6./4) * (he4nn_he6_g.__wrapped__(T)/2/(he6g_he4nn.__wrapped__(T) + he6_li6.__wrapped__(T)))*X[0][2]*X[0][5]
